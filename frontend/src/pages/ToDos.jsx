...
export default function ToDos({ archive }) {
    ...
    function updateToDoStatus(to_do_id, status) {
        axios.put(`https://${process.env.SELF_URL_P1}.praeficio.com/todos/${to_do_id}`, { completed: status }).then(response => {
    ...
    useEffect(() => { //main fetcher on load
        axios.get(`https://${process.env.SELF_URL_P1}.praeficio.com/todos/my_todos${archive ? '?archived=true' : ''}`).then(response => {
            if (response.status === 401) {
                setIsLoggedIn(false);
            } else if (response.status === 200) {
    ...
    return (<>
        ...
        {!archive && <Button
            ...
            onClick={() => {
                ...
                axios.post(`https://${process.env.SELF_URL_P1}.praeficio.com/todos/archive_all_completed`).then(response => {
                    if (response.status === 401) {
                        setIsLoggedIn(false);
            ...
            <Modal.Body>
                ...
                <Button
                    ...
                    onPress={async () => {
                        ...
                        await axios.post(`https://${process.env.SELF_URL_P1}.praeficio.com/todos/`, {
