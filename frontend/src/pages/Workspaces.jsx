...
export function MyWorkspacesSection({ setSelectedSubSection, favesOnly = false }) {
    ...
    useEffect(() => {
        axios.get(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces`).then(response => {
...
function WorkspaceCard({ details, invitationModalOpen, setSelectedDetails }) {
    ...
    return (
        <Card css={{ $$cardColor: '$colors$primary', width: '300px', height: '150px' }} key={innerDetails.workspace_id + '-card-inner'} isHoverable isPressable>
            <i
                onClick={(ev) => {
                    axios.put(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces/${innerDetails.workspace_id}/user/${user.id}`, { starred: true }).then(response => {
                        if (response.status === 200) {
...
function SpecificWorkspaceMessagesPanel({ user, setSelectedSubSection, selectedSubSection }) {
    ...
    useEffect(() => {
        axios.get(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces/${workspace_id}`).then(response => {
            if (response.status === 200) {
                ...
                <Row justify='space-evenly' >
                    ...
                    <CustomButton
                        ...
                        onClick={() => {
                            ...
                            axios.post(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspace_messages/${workspace_id}`, {
        ...
        <Modal.Body>
            ...
            <Button
                ...
                onPress={async () => {
                    ...
                    await axios.post(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces/`, {
            ...
            >
                ...
                <Button auto
                    ...
                    onPress={async () => {
                        ...
                        await axios.delete(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces/${selectedWorkspace.id}/user/${user.id}`).then(response => {
                ...
                <Button auto
                    ...
                    color="success"
                    onPress={async () => {
                        await axios.put(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/workspaces/${selectedWorkspace.id}/user/${user.id}`, { invitation_accepted: true }).then(response => {
                            if (response.status === 401) {
                                setIsLoggedIn(false);
                            } else if (response.status === 200) {
                                setModalOpen(false);
