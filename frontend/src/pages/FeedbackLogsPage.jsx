...
export default function FeedbackLogsPage({ archive }) {
    ...
    useEffect(() => {
        axios.get(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/feedback_logs${archive ? '?archived=true' : ''}`).then(response => {
            ...
            <Modal.Body>
                ...
                <Button
                    ...
                    onPress={async () => {
                        ...
                        await axios[selectedLog ? 'put' : 'post'](`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/feedback_logs/${selectedLog || ''}`, {
                            name: newLogName
                        }).then(response => {
                ...
                >
                    ...
                    <Button auto
                        ...
                        onPress={async () => {
                            ...
                            await axios.put(`https://${process.env.REACT_APP_BUILD_ENV}.praeficio.com/be/feedback_logs/${selectedLog}`, { archived: true }).then(response => {
                                if (response.status === 401) {
                                    setIsLoggedIn(false);
                                } else if (response.status === 200) {
                                    updateCachedItems(selectedLog, response.data.data);
