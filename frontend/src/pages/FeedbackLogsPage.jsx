...
export default function FeedbackLogsPage({ archive }) {
    ...
    useEffect(() => {
        axios.get(`/feedback_logs${archive ? '?archived=true' : ''}`).then(response => {
            ...
            <Modal.Body>
                ...
                <Button
                    ...
                    onPress={async () => {
                        ...
                        await axios[selectedLog ? 'put' : 'post'](`/feedback_logs/${selectedLog || ''}`, {
                            name: newLogName
                        }).then(response => {
                ...
                >
                    ...
                    <Button auto
                        ...
                        onPress={async () => {
                            ...
                            await axios.put(`/feedback_logs/${selectedLog}`, { archived: true }).then(response => {
                                if (response.status === 401) {
                                    setIsLoggedIn(false);
                                } else if (response.status === 200) {
                                    updateCachedItems(selectedLog, response.data.data);
