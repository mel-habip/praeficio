...
function Positions() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/user/${user.id}${query}`, {
    ...
    function togglePosition(position_id, turn_on_off) {
        axios.put(`http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/${position_id}/${turn_on_off ? 'reactivate' : 'deactivate'}`).then(response => {
            if (response.status === 401) {
                kickOut();
            } else if (response.status === 200) {
    ...
    function updateNotes(position_id, notes = []) {
        axios.put(`http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/${position_id}`, { notes: JSON.stringify(notes) }).then(response => {
            if (response.status === 401) {
                kickOut();
            } else if (response.status === 200) {
    ...
    function recoverPosition(position_id) {
        axios.put(`http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/${position_id}/recover`).then(response => {
            if (response.status === 401) {
                kickOut();
            } else if (response.status === 200) {
                    ...
                    <Button
                        ...
                        onPress={async () => {
                            ...
                            const options = {
                                ...
                                url: `http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/${isUpdate ? selectedPositionID : ''}`,
                            ...
                            >
                                ...
                                <Button auto
                                    ...
                                    onPress={async () => {
                                        ...
                                        await axios.delete(`http://${process.env.REACT_APP_BUILD_ENV}.praeficio.com:8000/positions/${selectedPositionID}`).then(response => {
                                            if (response.status === 401) {
                                                kickOut();
                                            } else if (response.status === 200) {
